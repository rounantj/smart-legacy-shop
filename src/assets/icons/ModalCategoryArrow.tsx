export default function ModalCategoryArrow() {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="36"
      height="36"
      viewBox="0 0 36 36"
    >
      <g
        id="Grupo_7565"
        data-name="Grupo 7565"
        transform="translate(36) rotate(90)"
      >
        <g
          id="Retângulo_390"
          data-name="Retângulo 390"
          fill="#fcfcfd"
          stroke="#edf2f6"
          stroke-width="1"
        >
          <rect width="36" height="36" rx="18" stroke="none" />
          <rect x="0.5" y="0.5" width="35" height="35" rx="17.5" fill="none" />
        </g>
        <g
          id="Componente_73_94"
          data-name="Componente 73 – 94"
          transform="translate(28 28) rotate(180)"
        >
          <g id="Grupo_1463" data-name="Grupo 1463" opacity="0.498">
            <g
              id="Elipse_4"
              data-name="Elipse 4"
              fill="none"
              stroke="#687c97"
              stroke-width="1"
            >
              <circle cx="10" cy="10" r="10" stroke="none" />
              <circle cx="10" cy="10" r="9.5" fill="none" />
            </g>
            <path
              id="arrow_next"
              data-name="arrow next"
              d="M581.273,789.774a.82.82,0,0,1-.081-1.079l.081-.092,3.685-3.593-3.685-3.593a.82.82,0,0,1-.081-1.079l.081-.093a.849.849,0,0,1,1.094-.081l.094.081,4.279,4.179a.82.82,0,0,1,.081,1.079l-.081.093-4.279,4.179A.849.849,0,0,1,581.273,789.774Z"
              transform="translate(795.009 -573.615) rotate(90)"
              fill="#687c97"
            />
          </g>
        </g>
      </g>
    </svg>
  );
}
